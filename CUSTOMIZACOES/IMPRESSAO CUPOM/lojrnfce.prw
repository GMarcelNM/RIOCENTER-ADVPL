...
User Function LOJRNFCe(	oNFCe		, oProt		, nDecimais	, aFormas	,;
	...
	Local aArea     := GetArea()                               // Pegar as areas padrãos - TOTVS NE 
    Local nPos      := 0                                       // Variavel de pesquisa - TOTVS NE
	Local lPrinter 	:= .F.			
	Local cXml		:= ""
	Local cXmlProt	:= ""
	Local cPath 			:= "\spool\"		
...
//Local aTroca        := {} 
 ...
 Local aVendedor := {}                                  // Matriz com os dados do vendedor - TOTVS NE
 Local aFormPg   := {}
 Local aFormPg2  := {}
 Local zZ        := 0
 Local nCredTroca := 0
...
Private aItemNFCe	:= {}	//dados dos itens
  ...
  // --- Pegar todos dos Vendeores (TOTVS)
  // -------------------------------------
   dbSelectArea("SL2")
   SL2->(dbSetOrder(1))
   SL2->(dbGoTop())
   ...
   If SL2->(dbSeek(xFilial("SL2") + SL1->L1_NUM))
      While ! SL2->(Eof()) .and. SL2->L2_FILIAL == SL1->L1_FILIAL .and. SL2->L2_NUM = SL1->L1_NUM
        ...
        If nPos == 0
           aAdd(aVendedor, {SL2->L2_VEND,;
           	                Posicione("SA3",1,xFilial("SA3") + SL2->L2_VEND,"A3_NREDUZ")})
	    EndIf      	              
 ...
 //------ Pegar formas de pagamento usada na venda (P3 tecnologia)
 //----------------
   dbSelectArea("SL4")
   SL4->(dbSetOrder(1))
   SL4->(dbGoTop())
   ...
   If SL4->(dbSeek(xFilial("SL4") + SL1->L1_NUM))
      While ! SL4->(Eof()) .and. SL4->L4_FILIAL == SL1->L1_FILIAL .and. SL4->L4_NUM = SL1->L1_NUM
        nPos := aScan(aFormPg, {|x| x[1] == SL4->L4_FORMA})
...
// ------------------
 ...
 //------ Pegar Possivel pagamento por credito de troca
 //----------------
  nCredTroca := SL1->L1_CREDITO
 //----------------
...
BEGIN SEQUENCE
	...
	If (lDanfePad .AND. lLJ7084) .Or. lSTImPFNfce
		...
		*/
        ...
        // iniciar mudança da impressão de pagamentos aqui 2021-08-11   @Glaudson Marcel


        /*  // CODIGO ORIGINAL ATE A MUDANCA
		...
		For nX := 1 to Len(aPagNFCe)
			...
			//If Alltrim(aPagNfce[nX]:_TPAG:TEXT) == "99"     // Venda com cartão Rio Center 
        ...
        */
		...
		For nX := 1 to Len(aPagNFCe)
    		...
    		Next zZ
   			if cFormaPgto = "" 
    			if nCredTroca == Val(aPagNfce[nX]:_VPAG:TEXT)
		...
		Next nX
///////////////////////////////////////////////////////////////////////////////
// fim mudança da impressão de pagamentos aqui 2021-08-11   @Glaudson Marcel	
//////////////////////////////////////////////////////////////////////////////
       ...
       // --- Impressão dos Vendedores / Operador - TOTVS NE
       // ---------------------------------------
       // --- Impressão dos Vendedores - TOTVS NE
       // ---------------------------------------
	//    cTexto += (TAG_CENTER_INI + cTagCondIni)
		cTexto += "Operador: " + SL1->L1_OPERADO + " " + AllTrim(Posicione("SA6",1,xFilial("SA6") + SL1->L1_OPERADO,"A6_NOME"))
	//	cTexto +=  (cTagCondFim + TAG_CENTER_FIM)
		cTexto += cCRLF

        For nX := 1 To Len(aVendedor)
	        cTexto += (TAG_CENTER_INI + cTagCondIni)
		    cTexto += "Vendedor: " + aVendedor[nX][01] + " " + AllTrim(aVendedor[nX][02])
		    cTexto +=  (cTagCondFim + TAG_CENTER_FIM)
		    cTexto += cCRLF
	    Next	
       // ---------------------------------------
		...
		*/
		...
		Else //Imprime Não Fiscal
			...
			Else
			    ...
			    If Len(aTroca) > 0
			    	For nR := 1 To Len(aTroca)
					
			    		cTexto += aTroca[nR][1]
				    	cTexto += TAG_CENTER_INI + TAG_CONDEN_INI +TAG_CODABAR_INI +AllTrim(aTroca[nR][2])+ TAG_CODABAR_FIM + TAG_CONDEN_FIM + TAG_CENTER_FIM
			    		cTexto += TAG_CENTER_INI + AllTrim(aTroca[nR][2])+ TAG_CENTER_FIM
			    		cTexto += aTroca[nR][3]
			    		cTexto += (TAG_GUIL_INI + TAG_GUIL_FIM)	//aciona a guilhotina
