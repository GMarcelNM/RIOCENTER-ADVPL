...
user function RStOrc()
	...
	Private cTabDesc := 'TRB'// Ponteiro para arquivo temporário
...
Static Function PrintReport(oReport,aParam)
	...
	oReport:SkipLine()
	oReport:PrintText("*** Período ...: " + aParam[1] + " a " + aParam[2] + " - " + cValtoChar(TRB->(RecCount())))
	oReport:SkipLine()
	oSection:Init()
...
//	Local cTab		:= 'TRB'// Ponteiro para arquivo temporário
    ...
    Aviso( dTos(aRet[1]), "Não existe dados a  consultar", {"Ok"} ) 
    Aviso( dTos(aRet[2]), "Não existe dados a  consultar", {"Ok"} ) 
    Aviso( aRet[3], "Não existe dados a  consultar", {"Ok"} ) 
...
//	cQuery	+= " AND L1_OPERADO IN " + FormatIn(cOperado,",") + " "
    ...
    Aviso( "teste 1", "Não existe dados a  consultar", {"Ok"} ) 

	If !Empty(aRet[3])
		cQuery	+= " AND L1_VEND = '" + aRet[3] + "' "
	ENDIF
    Aviso( "teste 2", "Não existe dados a  consultar", {"Ok"} ) 
	If !Empty(aRet[4])
    Aviso( cvaltochar(aRet[4]), " testando", {"Ok"} ) 
//		If aRet[4] == "Faturados"
		If aRet[4] == "1"
		...
		EndIf
	ENDIF
    Aviso( "passou", "Não existe dados a  consultar", {"Ok"} ) 
	If !Empty(aRet[5])
	    Aviso( aRet[5], "Não existe dados a  consultar", {"Ok"} ) 
		cQuery	+= " AND L1_FILIAL = '" + aRet[5] + "' "
	ENDIF
		    Aviso( "antes antes", "Não existe dados a  consultar", {"Ok"} ) 
...
//		dbCloseArea()
//	EndIf
		    Aviso( "antes", "Não existe dados a  consultar", {"Ok"} ) 

	dbUseArea(.T., 'TOPCONN', TCGenQry(,,cQuery), "cTabQry", .F., .T.)
		    Aviso( "depois", "Não existe dados a  consultar", {"Ok"} ) 
	
	Count to nQtdReg
	ProcRegua(nQtdReg)
	
	Aviso( cValToChar(nQtdReg), "Não existe dados a  consultar", {"Ok"} ) 
