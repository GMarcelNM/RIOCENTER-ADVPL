...
Static function MontDados(aParm)
	...
	TRFunction():New(oSection1:Cell("L1_VLRTOT")	,/* cID */,"SUM",/*oBreak*/,/*cTitle*/,'@e 999,999,999.99',/*uFormula*/,.T./*lEndSection*/,.F./*lEndReport*/,/*lEndPage*/)
	TRFunction():New(oSection1:Cell("L1_DESCONT")	,/* cID */,"SUM",/*oBreak*/,/*cTitle*/,'@e 999,999,999.99',/*uFormula*/,.T./*lEndSection*/,.F./*lEndReport*/,/*lEndPage*/)
	TRFunction():New(oSection1:Cell("L1_VLRLIQ")	,/* cID */,"SUM",/*oBreak*/,/*cTitle*/,'@e 999,999,999.99',/*uFormula*/,.T./*lEndSection*/,.F./*lEndReport*/,/*lEndPage*/)
...
Static Function PrintReport(oReport,aParam)
	...
	oReport:SkipLine()
	oReport:SkipLine()
    oReport:PrintText("*** Filial Selecionada ...: " + cFilialDesc)
	oReport:SkipLine()
//	oReport:PrintText("*** Período ...: " + aParam[1] + " a " + aParam[2])
	oReport:PrintText("*** Período ...: " + cData1 + " a " + cData2)
	oReport:SkipLine()
	oSection:Init()
...
Static Function LeDados()
	...
	cQuery	+= " INNER JOIN " + RETSQLNAME("SA3") + " A3 ON A3.D_E_L_E_T_ <> '*' AND A3_COD = L1_VEND AND A3_FILIAL = L1_FILIAL "
	cQuery	+= " INNER JOIN " + RETSQLNAME("SA1") + " A1 ON A1.D_E_L_E_T_ <> '*' AND A1_COD = L1_CLIENTE AND A1_LOJA = L1_LOJA "
	cQuery	+= " WHERE L1.D_E_L_E_T_ <> '*' "
	cQuery	+= " AND L1_EMISSAO BETWEEN '" + cData1 + "' AND '" + cData2 + "' "
//	cQuery	+= " AND L1_XUSER IN " + FormatIn(cOperado,",") + " "
	If !Empty(aRet[3])
